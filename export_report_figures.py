...
def build_multi_event_timeline_figure(
    ...
    """Line chart showing daily report volume for all tracked events."""
    ...
    fig = px.line(
        ...
        color="event_label",
        markers=True,
        title=f"Multi-Event Timeline – Daily News Volume ({rolling_window}-day rolling avg)",
        labels={
            "pub_date": "Date",
...
def build_resilience_radar_figure(
    ...
    """Radar chart comparing impact and media metrics across all events."""
        ...
        metrics.append(
            {
                ...
                "Vulnerability proxy": float(
                    row.get("economic_damage_per_capita") or 0
                ),
            }
        )
    ...
    for _, row in norm_df.iterrows():
        ...
        fig.add_trace(
            go.Scatterpolar(
                ...
                opacity=0.6,  # Reduced opacity so overlapping 4 events is readable
            )
        )
    fig.update_layout(
...
def build_sentiment_over_time_figure(media_enriched: pd.DataFrame) -> go.Figure:
    """Line chart tracking sentiment volatility across the events."""
...
def build_fci_and_coverage_comparison_figure(
    ...
    """Grouped bar chart: FCI and reports-per-USD damage by event."""
    ...
    fig.add_trace(
        go.Bar(
            name="FCI (reports per affected person)", x=df["event_label"], y=df["FCI"]
        ),
        secondary_y=False,
    )
    fig.add_trace(
        go.Bar(
            name="Reports per USD damage (×1e-8)",
            x=df["event_label"],
            y=df["Reports_per_USD_1e8"],
        ),
        secondary_y=True,
    )
...
def build_sentiment_distribution_figure(media_enriched: pd.DataFrame) -> go.Figure:
    ...
    if "headline_sentiment_compound" not in media_enriched.columns:
        return go.Figure()
    df = media_enriched.dropna(subset=["event_label", "headline_sentiment_compound"])[
        ["event_label", "headline_sentiment_compound"]
    ]
    if df.empty:
        return go.Figure()
    ...
    fig = px.box(
        ...
        labels={
            "event_label": "Event",
            "headline_sentiment_compound": "Sentiment (compound)",
        },
    )
    fig.update_layout(xaxis_tickangle=-25, margin=dict(b=100), showlegend=False)
    return fig
...
def build_delta_t_comparison_figure(summary: Dict[str, Any]) -> go.Figure:
    ...
    fig = px.bar(
        ...
        labels={"x": "Event", "y": "Days (ΔT)", "color": "Event"},
    )
...
def main():
    ...
    figures = [
        ("multi_event_timeline", build_multi_event_timeline_figure(media_enriched)),
        ...
        (
            "fci_and_coverage_comparison",
            build_fci_and_coverage_comparison_figure(impact_df, media_events_df),
        ),
    ...
    print("=" * 60)
    print(f"Done. Figures are available in {REPORT_FIGURES_DIR}/")
